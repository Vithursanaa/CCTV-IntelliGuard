...
def send_telegram_message(text):
    """Send alert text message to Telegram with timeout and error handling"""
    url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage"
    data = {"chat_id": CHAT_ID, "text": text}
    try:
        r = requests.post(url, data=data, timeout=10)  # 10s timeout
        if r.status_code == 200:
            print("[INFO] Telegram message sent successfully.")
        else:
            print(f"[WARN] Telegram returned {r.status_code}: {r.text}")
    except requests.exceptions.Timeout:
        print("[ERROR] Telegram message timed out.")
    except requests.exceptions.RequestException as e:
        print(f"[ERROR] Telegram message failed: {e}")

def send_telegram_video(video_path, caption="Human Detected!"):
    """
    Send recorded event video to Telegram:
    - Checks file existence
    - Compresses large videos (>49MB)
    ...
    # Compress video if larger than 49 MB
    size_mb = os.path.getsize(video_path) / (1024 * 1024)
    if size_mb > 49:
        compressed_path = video_path.rsplit('.', 1)[0] + "_compressed.mp4"
        print(f"[INFO] Compressing video ({size_mb:.1f} MB) to {compressed_path} ...")
        subprocess.run([
    ...
    # Try sending video up to 3 times
        ...
        try:
            with open(video_path, "rb") as video_file:
                r = requests.post(
                    ...
                    timeout=60  # long timeout for larger files
                )
            if r.status_code == 200:
                print("[INFO] Telegram video sent successfully.")
                return
            else:
                print(f"[WARN] Telegram API returned {r.status_code}: {r.text} (attempt {attempt+1}/3)")
        except requests.exceptions.Timeout:
            print(f"[ERROR] Telegram video timed out (attempt {attempt+1}/3)")
        except requests.exceptions.RequestException as e:
            print(f"[ERROR] Telegram video failed (attempt {attempt+1}/3): {e}")
        time.sleep(5)  # wait before retry

    print("[FAIL] Could not send Telegram video after 3 attempts.")
